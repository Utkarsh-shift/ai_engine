import numpy as np
import pandas as pd
def AI_resultGenerator(data):
    print("in AI result Generator ----------------------------------------------------------",data)
    try:
        base_language_feedback =f"""     
                ...
                - The Audio Sentiment Analysis result is {data["sentiment_score"]}, comment on the sentiment score of the candidates.
                - The Audio Energy Analysis result is {data["energy_score"]}, comment on the Energy Score of the candidates.
                - The Audio Pace Analysis result is {data["pace_score"]}, comment on the Pace Score of the candidates.
                - The Audio Pace_and_Clarity Analysis result is {data["pace_and_clarity_score"]}, comment on the Pace_and_Clarity Score of the candidates.
                - The Audio Articulation Analysis result is {data["articulation_score"]}, comment on the Articulation Score of the candidates.
                - The Audio Communication Analysis result is {data["communication_score"]}, comment on the Communication Score of the candidates.
                - The Sociability Score is {data["sociability_score"]}, comment on the Sociability Score of the candidates.
                - The Positive Attitude is {data["positive_attitude"]}, comment on the Positive Attitude Score of the candidates.
                - The Dressing Score is {data["professional_score"]}, comment on the Dressing Score of the candidates.
                - The Body language Score is {data["body_language_score"]}, comment on the body language score of the candidates.
                - The Grammer Score is {data["grammer"]}, comment on the grammer Score of the candidates.
                - The Emotion Score is {data["emotion_score"]}, comment on the emotion Score of the candidates.
                - The Presentablity Score is {data["presentabilty_score"]}, comment on the presentiablity Score of the candidates.
                - The Overall Professional Score is {data["overall_professional_Score"]}, comment on the Overall Professional Score of the candidates.
                - The Overall Score is {data["overall_Score"]}, comment on the Overall Score Score of the candidates. """
    except:
        datawd = pd.to_numeric(data['df_bigram']['Frequency'], errors='coerce')
        datadw = pd.to_numeric(data['df_unigram']["Frequency"],errors="coerce")
        base_language_feedback =f"""    
        As an AI video interviewer scorer, provide detailed, critical feedback on the candidate's performance by :
        - The big Five train values calculates were {data['ocean_values'][0]}, {data['ocean_values'][1]}, {data['ocean_values'][2]}, {data['ocean_values'][3]} and {data['ocean_values'][4]} suggest where the candidate lacks and suggest some solutins accordingly.
        - The bigram and unigram words along with the frequency is calculated as {datawd[:2]},{datadw[:2]}, comment on the candidates vocabulary. High-frequency terms may indicate what the candidate emphasizes or considers important.Analyze the connection between words (bigrams) to understand how the candidate links concepts and ideas.Frequent repetitions or contradictions within the word pairs might suggest confusion or a lack of knowledge about the topic discussed.
        - The Audio Sentiment Analysis result is {data["sentiment_score"]}, comment on the sentiment score of the candidates.
        - The Audio Energy Analysis result is {data["energy_score"]}, comment on the Energy Score of the candidates.
        - The Audio Pace Analysis result is {data["pace_score"]}, comment on the Pace Score of the candidates.
        - The Audio Pace_and_Clarity Analysis result is {data["pace_and_clarity_score"]}, comment on the Pace_and_Clarity Score of the candidates.
    ...
    - Try to dig deeper into the most important parts of the candidate's data['mean_Action_unit'] = data['mean_Action_unit'].values.tolist()
solution by asking questions about different parts of the solution.
    ...
    """
    # data['mean_Action_unit'] = data['mean_Action_unit']
    ...
    # - The candidate's headpose direction throughout the interview is {data['headPose']}. Specify whether the candidate is focusing on camera or elsewhere?
    # - The candidate's Eyegazed direction throughout the interview is provided as 'blink': {data['blink']}, 'Looking center': {data['lookcentre']}, 'Looking right':{data['lookright']} and 'looking left': {data['lookleft']}. Determine whether the candidate is focused throughout the interview or not?
    # """
    base_video_feedback = f""""
    As an AI Audio interview scoring, provide detailed, critical feedback on the candidate's speaking performance by:
    - The 'inner brow raiser' mean is calculated as {data['mean_Action_unit'][0]}, comment whether the person is showing engagement or not.
    ...
    - the 'Nasolabial deepener' mean is calculated as {data['mean_Action_unit'][8]}, comment whether the person is often associated with negative emotions such as sadness, or contempt.
    - the 'Lip corner puller' mean is calculated as {data['mean_Action_unit'][9]}, comment on the person's positive emotions such as happiness, amusement and a genuine smile.
    - the 'Dimpler' mean is calculated as {data['mean_Action_unit'][10]}, comment on whether the person is showing or reflecting a more complex or mixed emotions.
    - the 'lip corner depressor' mean is calculated as {data['mean_Action_unit'][11]}, comment on whether the person is showing negative emotions such as sadness, dissatisfaction, or contempt.
    - the 'chin raiser' mean is calculated as {data['mean_Action_unit'][12]}, comment whether a person is showing a mixed or negative emotions.
    - the 'Lip Stretcher' mean is calculated as {data['mean_Action_unit'][13]}, comment whether the person is suffering from tension, fear and anxiety.
    - the 'lip tightener' mean is calculated as {data['mean_Action_unit'][14]}, comment whether the person is showing negative emotions.
    - the 'lip pressor' mean is calculated as {data['mean_Action_unit'][15]}, comment whether the person is associated with stress, determination, restraint or surpressed anger.
    - the 'lips part' mean is calculated as {data['mean_Action_unit'][16]}, comment on whether the person's emotion is associated with surpurise,fear, interest or readiness to speak.
    - the 'jaw drop' mean is calculated as {data['mean_Action_unit'][17]}, comment on whether the person is surprise, shock, amazement, or fear.
    - the 'lip suck' mean is calculated as {data['mean_Action_unit'][18]}, comment on whether the person expression is associated with fear, shock,or extreme effort.
    - the 'eyes closed' mean is calculated as {data['mean_Action_unit'][19]}, comment on person's behaviour.
    - The candidate's headpose and Eyegazed direction throughout the interview is provided as  "headleft_watchleft" : {data["headleft_watchleft"]},"headleft_watchright": {data["headleft_watchright"]},"headleft_watchcentre":{data["headleft_watchcentre"]},"headright_watchleft":{data["headright_watchleft"]},"headright_watchright": {data["headright_watchright"]},"headright_watchcentre": {data["headright_watchcentre"]},"headdown_watchleft": {data["headdown_watchleft"]},"headdown_watchright": {data["headdown_watchright"]},"headdown_watchcentre":{data["headdown_watchcentre"]},"headup_watchleft":{data["headup_watchleft"]},"headup_watchright":{data["headup_watchright"]},"headup_watchcentre":{data["headup_watchcentre"]},"headstraight_watchleft":{data["headstraight_watchleft"]},"headstraight_watchright":{data["headstraight_watchright"]},"headstraight_watchcentre":   {data["headstraight_watchcentre"]}   . Specify whether the candidate is focusing on camera or elsewhere and also specify whether the candidate is focused throughout the interview or not?     
   ...
   # print("The prompt use is given as ##############################################################################################################", prompts)
    return prompts