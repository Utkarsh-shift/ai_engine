...
class AudioVisualData(VideoData):
    ...
    def get_wave_data(self, idx):
        print("changes have to be done ")
        print("The self.test only is given as",self.test_only)
        if self.test_only :
            img_dir_path = self.img_dir_ls[idx]
            dir_name = os.path.basename(img_dir_path)
            wav_path = img_dir_path.replace(dir_name, "voice_data/voice_librosa/")
            wav_path = "./"+wav_path.replace("/test_data","")
            wav_path = wav_path + "test_data/"
            wav_path = wav_path  + dir_name + ".wav.npy"
            print(wav_path)
            try:
        ...
        # dir_name = os.path.basename(img_dir_path)
        # wav_path = img_dir_path.replace(dir_name, "voice_data/voice_librosa/")
        # wav_path = "./"+wav_path.replace("/train_data","")
        # wav_path = "./"+wav_path.replace("/train_data","")
        # try:
        #     print("here " , wav_path)
        #     wav_ft = np.load(wav_path)
        ...
        #     dir_name = os.path.basename(img_dir_path)
        #     wav_path = img_dir_path.replace(dir_name, "voice_data/voice_librosa/") 
        #     wav_path = "./"+wav_path.replace("/valid_data","")
        #     wav_path = wav_path +"valid_data/" + dir_name + ".wav.npy"
        #     try:
        #         print("here 2" , wav_path)
        #         wav_ft = np.load(wav_path)
        #     except: 
        ...
        # wav_path = img_dir_path.replace(dir_name, "voice_data/voice_librosa/")
        # wav_path = "./"+wav_path.replace("/test_data","")
        # print("wav pth",wav_path)
        # wav_path = wav_path +"test_data/" + dir_name + ".wav.npy"
        # print("The wav_path is " , wav_path)
      #  wav_path="/home/almabay/Documents/AVI-PA/DeepPersonality/Interviewer/datasets/ChaLearn/voice_data/voice_librosa/test_data/test.wav.npy"
      #  wav_ft = np.load(wav_path)
