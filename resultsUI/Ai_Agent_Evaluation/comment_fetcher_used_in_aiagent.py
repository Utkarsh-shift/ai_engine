...
MAX_TOKENS = 10000
OUTPUT_TOKENS = 300
from resultsUI.Ai_Agent_Evaluation.dictionary_used_in_agent import INTERVIEW_GOALS_LIST
...
def convert_images_to_base64(image_folder_path):
    if not os.path.isdir(image_folder_path):
        raise FileNotFoundError(f"{image_folder_path} is not a valid directory or does not exist.")

    image_folder = os.listdir(image_folder_path)
    image_folder.sort(key=lambda f: int(''.join(filter(str.isdigit, f))))  # Sort files based on numeric values in filenames
    base64Frames = []

    for filename in image_folder:
        file_path = os.path.join(image_folder_path, filename)

        # Only process valid image files (jpg, jpeg, png)
        if os.path.isfile(file_path) and filename.lower().endswith(('.png', '.jpg', '.jpeg')):
            image = cv2.imread(file_path)
            _, buffer = cv2.imencode('.jpg', image)
            base64_str = base64.b64encode(buffer).decode('utf-8')
...
def evaluate_self_awareness_combined(questions, transcripts):
    print("_+++++++++++_+_+_++3333333333333333333333333333333333333333+++++_+_+_+")
    print(questions)
    print("=======================")
    print(transcripts)
    total_score = 0
    ...
    for question, transcript in zip(questions, transcripts):
        messages = [
            {
                ...
                "content": (
                    ...
                    "Now, provide an evaluation based on the following question and answer:\n"
        ...
        try:
            # Sending request to OpenAI for response
            response = client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=messages,
                max_tokens=400,
            ...
            # Scaling the score to range [30, 100]
            # Adjust according to the model's scoring behavior
            scaled_score = ((score - 1) / 9) * (100 - 30) + 30  # Assuming original score range is 1-10
            scaled_score = round(scaled_score)

            # Extracting self-awareness comment
            ...
            all_comments.append(f"Error processing response: {str(e)}")

    # Calculating average score and combining all comments
    average_score = round(total_score / count) if count > 0 else 0
    combined_comment = " ".join(all_comments)

    return {
        "score": average_score,
        "self_awareness": combined_comment
    }
